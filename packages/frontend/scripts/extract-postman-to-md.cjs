...
function usage() {
  console.log('Usage: node extract-postman-to-md.cjs --input <postman.json> --output <out.md>')
}
...
for (let i = 0; i < argv.length; i++) {
  const a = argv[i]
  if (a === '--input' || a === '-i') inputPath = argv[i+1], i++
  else if (a === '--output' || a === '-o') outputPath = argv[i+1], i++
}
