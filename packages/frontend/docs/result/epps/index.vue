      ...
      <div class="card-body">
        ...
        >
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <button
              v-if="selectedDataset?.orderItemId"
              class="btn btn-sm btn-success gap-2"
              :class="{ 'btn-disabled': verifying }"
              :disabled="verifying"
              @click="verifyResult"
            >
              <i-tabler-check v-if="!verifying" class="w-4 h-4" />
            ...
            <button
              ...
              :class="{ 'btn-disabled': downloadingPdf }"
              :disabled="downloadingPdf"
              @click="downloadPdf"
            >
              <i-tabler-download v-if="!downloadingPdf" class="w-4 h-4" />
...
// State untuk verifikasi dan download
...
const verifyResult = async () => {
  const orderItemId = selectedDataset.value?.orderItemId;
  if (!orderItemId) {
    console.error('orderItemId tidak ditemukan pada dataset');
    return;
  }

  verifying.value = true;
  try {
    await api(`/outpatient/test-orders/result-verifications/${orderItemId}`, {
      method: 'POST'
    });
    console.log('Hasil test EPPS berhasil diverifikasi');
...
const downloadPdf = async () => {
  const orderItemId = selectedDataset.value?.orderItemId;
  if (!orderItemId) {
    console.error('orderItemId tidak ditemukan untuk download PDF');
    return;
  }

  downloadingPdf.value = true;
  try {
    // Endpoint untuk download PDF hasil test
    const response = await api(`/outpatient/test-orders/result-pdf/${orderItemId}`, {
      method: 'GET',
      responseType: 'blob'
    });
    ...
    link.href = url;
    link.download = `hasil-epps-${selectedDataset.value?.__regCode || 'result'}.pdf`;
