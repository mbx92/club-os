...
5. taxBase = subtotalAfterDiscount + serviceCharge
6. tax = taxBase × taxRate% (if enabled)
7. totalAmount = subtotalAfterDiscount + serviceCharge + tax
```
...
function calculateOrderTotal(items, voucherDiscount = 0) {
  ...
  // 3. Calculate service charge (percentage)
  let serviceCharge = 0;
...
function calculateCombinedTotal(items, voucherDiscount = 0) {
  ...
  // 3. Service charge ONLY for restaurant items
  let serviceCharge = 0;
  ...
  // 4. Tax for all (subtotal after discount + service charge)
  let tax = 0;
  if (taxConfig.taxEnable) {
    const taxBase = subtotalAfterDiscount + serviceCharge;
    if (taxConfig.taxType === 'percentage') {
  ...
  const calculateTotal = () => {
    ...
    if (settings.taxConfig.taxEnable) {
      ...
      if (settings.taxConfig.taxType === 'percentage') {
        tax = (taxBase * settings.taxConfig.taxPercentage) / 100;
      } else {
...
### Issue 4: Tax calculated before service charge

**Solution**: Correct order is: subtotal → voucher → service charge → tax. Tax base includes service charge.
