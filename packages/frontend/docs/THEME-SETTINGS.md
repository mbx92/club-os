...
**Notes:**
- Called automatically after login if `user.tenant.settings` not present
- Used to fetch fresh tenant settings including theme

### 2. PATCH `/api/tenants/settings`
...
### 3. GET `/api/auth/user` (existing) OR Login Response
