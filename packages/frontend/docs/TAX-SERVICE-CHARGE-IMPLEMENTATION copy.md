...
4. tax = subtotalAfterDiscount × taxRate% (if enabled)
5. serviceCharge = subtotalAfterDiscount × serviceChargeRate% (if enabled)
6. totalAmount = subtotalAfterDiscount + tax + serviceCharge
```
...
function calculateOrderTotal(items, voucherDiscount = 0) {
  ...
  // 3. Calculate tax (from subtotal only, NOT including service charge)
  let tax = 0;
  ...
  // 4. Calculate service charge (percentage)
  let serviceCharge = 0;
  ...
  // 5. Calculate total
  const totalAmount = Math.round(subtotalAfterDiscount + tax + serviceCharge);
...
function calculateCombinedTotal(items, voucherDiscount = 0) {
  ...
  // 3. Tax for all (from subtotal only, NOT including service charge)
  let tax = 0;
  if (taxConfig.taxEnable) {
    if (taxConfig.taxType === 'percentage') {
      tax = (subtotalAfterDiscount * taxConfig.taxPercentage) / 100;
  ...
  // 4. Service charge ONLY for restaurant items
  let serviceCharge = 0;
...
function OrderSummary({ items, voucherDiscount = 0 }) {
  ...
  const calculateTotal = () => {
    ...
    // Tax (from subtotal only, NOT including service charge)
    let tax = 0;
    ...
    return {
      ...
      serviceCharge: Math.round(serviceCharge),
      tax: Math.round(tax),
      total: Math.round(subtotalAfterDiscount + tax + serviceCharge)
    };
  };
...
**Solution**: Tax should be calculated from subtotal after discount only, NOT including service charge. Correct order is: subtotal → voucher → tax (from subtotal) → service charge → total.
